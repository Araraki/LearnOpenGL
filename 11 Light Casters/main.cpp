...
{	
	...
	// texture
	TextureManager::Inst()->LoadTexture("box.png", 0, GL_RGBA, GL_RGBA, 0, 0);
	...
	TextureManager::Inst()->LoadTexture("box_specular.png", 1, GL_RGBA, GL_RGBA, 0, 0);
	glGenerateMipmap(GL_TEXTURE_2D);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform3f(glGetUniformLocation(depthShader.Program, "viewPos"), camera.Position.x, camera.Position.y, camera.Position.z);
		glUniform4f(glGetUniformLocation(depthShader.Program, "light.lightVector"), lightVector.x, lightVector.y, lightVector.z, lightVector.w);

		glUniform3f(glGetUniformLocation(depthShader.Program, "light.spotDir"), spotDir.x, spotDir.y, spotDir.z);
		glUniform1f(glGetUniformLocation(depthShader.Program, "light.cutOff"), cutOff);		
		glUniform1f(glGetUniformLocation(depthShader.Program, "light.outerCutOff"), outerCutOff);

		glUniform1f(glGetUniformLocation(depthShader.Program, "light.constant"), 1.0f);
		glUniform1f(glGetUniformLocation(depthShader.Program, "light.linear"), 0.09f);
		glUniform1f(glGetUniformLocation(depthShader.Program, "light.quadratic"), 0.032f);
				
		//shader.lightParameter		
		glUniform3f(glGetUniformLocation(depthShader.Program, "light.ambient"), 0.2f, 0.2f, 0.2f);
		glUniform3f(glGetUniformLocation(depthShader.Program, "light.diffuse"), 0.5f, 0.5f, 0.5f);
		glUniform3f(glGetUniformLocation(depthShader.Program, "light.specular"), 1.0f, 1.0f, 1.0f);

		// shader.material
		glUniform1i(glGetUniformLocation(depthShader.Program, "material.diffuse"), 0);
		glUniform1i(glGetUniformLocation(depthShader.Program, "material.specular"), 1);
		glUniform1i(glGetUniformLocation(depthShader.Program, "material.emission"), 2);
		glUniform1f(glGetUniformLocation(depthShader.Program, "material.shininess"), 64.0f);
